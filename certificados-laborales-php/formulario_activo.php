...
<head>
    <meta charset="UTF-8">
    <title>Certificado Trabajador Activo</title>
...
<body>
    <div class="container">
        ...
        <form action="generar.php" method="post">
            ...
            <input type="text" placeholder= "Sin puntos" name="cedula" id="cedula" value="<?= htmlspecialchars($_POST['cedula'] ?? '') ?>" required>
...
<select name="motivo" id="motivo" required>
    <option value="">-- Seleccione un motivo --</option> <option value="Arriendo" <?= (($_POST['motivo'] ?? '') === 'Arriendo') ? 'selected' : '' ?>>Arriendo</option>
    <option value="Préstamo" <?= (($_POST['motivo'] ?? '') === 'Préstamo') ? 'selected' : '' ?>>Préstamo</option>
    <option value="Estudio" <?= (($_POST['motivo'] ?? '') === 'Estudio') ? 'selected' : '' ?>>Estudio</option>
</select>
            
            <label for="solicitud">A solicitud de:</label>
            <input type="text" name="solicitud" id="solicitud" placeholder="Ejm: Bancolombia" value="<?= htmlspecialchars($_POST['solicitud'] ?? '') ?>" required>

            <button type="submit">Generar Certificado</button>
        </form>

        <a href="login_admin.php" class="admin-link">Acceso para Administradores</a>
        <a href="formulario.php" class="admin-link">Menú principal</a>
    </div>
</body>
</html>